...
\end{equation}
其中 $X$ 和 $Y$ 为不同的输入表示。交叉注意力用于建模不同特征空间或不同声学表示之间的交互关系\cite{zou2022speech,he2023multiple}。
...
在语音情绪识别中，加性注意力可应用于循环神经网络模型的输出或局部隐藏状态集合，用于强调与情绪判断相关性较高的时刻\cite{mirsamadi2017automatic}。
