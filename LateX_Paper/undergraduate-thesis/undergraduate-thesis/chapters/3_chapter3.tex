...
在语音情绪识别中, 有研究指出情绪信息可能以不均匀的方式分布于整个发话, 相对集中于特定的时间区间、特定的频率范围或特定的声学模式之中\cite{mirsamadi2017automatic, liu2023discriminative, zou2022speech, he2023multiple}. 卷积神经网络模型和循环神经网络模型可用于学习语音表示；在采用统一权重处理输入各部分时，模型可能难以充分区分与情绪判断相关性相对较高的区间\cite{zhang2023cnn, liu2023dualrobustness, peng2021efficient}. 注意力机制作为一种计算输入内部相关性并据此调整信息组合权重的结构方法，被引入语音情绪识别中\cite{mirsamadi2017automatic, zou2022speech}. 注意力机制通过相关性计算与加权聚合来重新分配信息重要性，既包括基于查询、键、值计算输入元素之间关系的注意力结构，也包括根据查询与键-值来源不同区分的自注意力和交叉注意力；根据注意力分数的计算方式，可使用缩放点积注意力或加性注意力；根据结构的并行化或作用范围，可使用多头注意力或局部注意力等扩展形式\cite{mirsamadi2017automatic, he2023multiple, chen2023dwformer, wang2024swin}.
...
注意力机制可理解为一种计算输入序列或特征图中各元素之间相关性，并根据计算结果调整信息组合权重的通用方法\cite{mirsamadi2017automatic,zou2022speech}。在语音情绪识别中，情绪线索并非均匀分布于整个发话，通常相对集中于特定时间区间、特定频率范围或特定声学模式\cite{mirsamadi2017automatic,liu2023discriminative,he2023multiple}。基于这一特性，注意力机制通过计算输入内部元素之间的关联程度，并据此重新分配信息重要性，从而更有选择性地反映与情绪判断相关性较高的区间\cite{mirsamadi2017automatic,zou2022speech}。
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.97\textwidth]{images/chapter3/chapter3_attention_qkv.drawio.pdf}
...
加性注意力在分数计算后的归一化与加权聚合过程，与式 \eqref{eq:attn_general_alpha} 和式 \eqref{eq:attn_general_output} 所示的一般注意力形式一致。与缩放点积注意力相比，加性注意力的主要差异体现在注意力分数构造阶段：该方法使用显式线性变换与非线性函数计算分数\cite{mirsamadi2017automatic}。
...
Transformer结构的一个特点是，它不像循环神经网络模型那样按时间步顺序更新状态。循环神经网络模型依赖上一时刻的隐藏状态计算当前时刻的表示，整个序列处理本质上是顺序的。Transformer可以在同一层内同时计算序列中多个位置之间的相关性，有利于并行计算\cite{vaswani2017attention}。
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.97\textwidth]{images/chapter3/chapter3_transformer_encoder.drawio.pdf}
...
\end{equation}
其中 $x \in \mathbb{R}^{d}$ 为某位置的输入向量，$\mu$ 和 $\sigma^2$ 分别为该向量的均值和方差，$\epsilon$ 为防数值不稳定的小常数。$\gamma$ 和 $\beta$ 为可学习的缩放与平移参数，$\odot$ 表示逐元素乘法。层归一化在每个位置的表示向量内部进行归一化，对序列长度和批次大小的变化相对不敏感\cite{vaswani2017attention}。
...
Transformer的第一个特点在于它相对有利于并行计算。循环神经网络模型中，当前时刻的计算依赖于前一时刻的状态，整个处理过程通常是顺序的。Transformer通过自注意力机制可以同时计算多个位置之间的相关性，在输入长度相同时，其计算结构可以相对并行化地组织\cite{vaswani2017attention}。当发话长度增加到一定程度，或采用帧级、补丁级输入时，这一并行计算特性可能在一定程度上简化计算流程\cite{chen2023dwformer,wang2024swin}。
...
Transformer的优势表现与具体条件有关。随着输入长度增加，注意力计算范围扩大，计算量和内存占用可能相应增加\cite{vaswani2017attention}。在语音情绪识别中，帧级序列可能较长；若采用高分辨率频谱表示或多特征融合，输入标记数量会进一步增多。在此情况下，应用原始Transformer结构不一定总是高效的选择，实际研究中已提出基于窗口的注意力、移位窗口、分层补丁表示等结构调整方法\cite{chen2023dwformer,wang2024swin}。
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\textwidth]{images/chapter3/chapter3_transformer_ser_boundary.drawio.pdf}
